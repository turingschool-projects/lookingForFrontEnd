...
class JobIndex extends React.Component {
  ...
  getAllRecentJobs() {
    let request = new Request(
      `https://lookingforme.herokuapp.com/api/v1/recent_jobs?page=${this.state.activePage}`,
      { method: 'GET' }
    );
  ...
  getFilteredJobs(parameter, searchInput) {
    let request = new Request(
      `https://lookingforme.herokuapp.com/api/v1/recent_jobs?${parameter}=${searchInput}&page=${this.state.activePage}`,
      { method: 'GET' }
    );
