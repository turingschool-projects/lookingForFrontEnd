...
class JobShow extends React.Component {
  ...
  render() {
    return (
      <Job key={this.state.job.id} job={this.state.job} full={true}/>
    );
  }
}
    ...
    return(
      <div>
        { this.state.jobs.map((job) => {
          return <Job key={job.id} job={job} full={false}/>;
        })}
      </div>
    );
  }
...
const Job = ({job, full}) => {
  ...
  return(
      <div className="jobListing">
        ...
        <p>{full ? job.description : trimBody(job.description, 100)}</p>
        ...
        {full ? "" : <button><a href="/#/job/1">View Details</a></button>}
        {full ? <a href="/">Back to all jobs</a> : ""}
      </div>
      )
    }
}
