...
class JobListItem extends React.Component {
  constructor(props) {
    super(props);
    this.setFavoritesToTrue = this.setFavoritesToTrue.bind(this)
    this.setFavoritesToFalse = this.setFavoritesToFalse.bind(this)
    ...
    if (typeof this.props.job.title !== "undefined") {
      return (
        <div className="container job-listing row">
          ...
          <div className="col-sm-4 column">
            <div className="content">
              <Link to={`/jobs/${this.props.job.id}`} className="btn btn-default btn-sm">View Details</Link><br></br><br></br>
  ...
  renderFavoritesButton() {
    if (!this.state.isFavorite) {
      return (
        <div>
          <button className="btn btn-default btn-sm" onClick={this.setFavoritesToTrue}>Add To Favorites</button>
        </div>
      )
    }
    return (
      <div>
        <button className="btn btn-default btn-sm" onClick={this.setFavoritesToFalse}>Remove From Favorites</button>
      </div>
    )
  }
