...
class JobIndex extends React.Component {
  ...
  getFavoriteJobs(recentJobs) {
    if(this.state.jobs && localStorage.favoriteJobs) {
      let favoriteJobs = this.state.jobs.filter((job) => {
        localStorage.favoriteJobs.includes(job)
      })
      this.setState({ jobs: favoriteJobs, lastCall: "favorites" })
    }
  }
    ...
    return (
      <div>
        ...
        <div className="container">
          <SearchBar getFilteredJobs={this.getFilteredJobs.bind(this)} getFavoriteJobs={this.getFavoriteJobs.bind(this, recentJobs)} />
          {(!recentJobs) ?
            "Loading..." :
